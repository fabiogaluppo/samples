...
struct tsp_class final
{
	...
	//calculate 2D euclidean distance from this circuit
	...
	static double euclidean_distance(city_location c1, city_location c2)
	{
		...
		auto x2 = c2.first;
		auto y2 = c2.second;
		auto dx = std::abs(static_cast<double>(x1 - x2));
		auto dy = std::abs(static_cast<double>(y1 - y2));
