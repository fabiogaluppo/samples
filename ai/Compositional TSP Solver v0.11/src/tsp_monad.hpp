...
struct tsp_monad final
{
	...
	static friend auto ref(const tsp_monad& x) -> tsp_class& //keep this member?
	{
		return ::ref(x.value);
	}
	
	static friend auto has(const tsp_monad& x) -> bool
	{
...
auto unit(const tsp_monad::value_type& value) -> tsp_monad
{
	tsp_monad result;
	result.value = value;
