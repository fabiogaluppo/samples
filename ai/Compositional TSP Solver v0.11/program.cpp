...
void MonadicLawsProofing()
{
	...
	//Scala notation: m flatMap f flatMap g == m flatMap (x => f(x) flatMap g)
	{
		...
		auto f = [](tsp_monad::value_type& t) -> tsp_monad
		{
			auto u = ref(t);
		...
		auto g = [](tsp_monad::value_type& t) -> tsp_monad
		{
			auto u = ref(t);
		...
		auto fun_result_2 = bnd(m, [&](tsp_monad::value_type& t) {
			return bnd(f(t), g);
		});

		auto oo_result_1 = m.map(f).map(g);
		auto oo_result_2 = m.map([&](tsp_monad::value_type& t) {
			return f(t).map(g);
		});
